...
export class ExportService {
  ...
  async exportToExcel(
    ...
    // ──────────────────────────────────────────────────────────────────────
    // Use buffer-based ExcelJS.Workbook (NOT streaming WorkbookWriter).
    // The streaming approach corrupts the ZIP/OOXML structure because
    // NestJS interceptors (ClassSerializerInterceptor, LoggingInterceptor)
    // can interfere with the chunked response stream.
    // ──────────────────────────────────────────────────────────────────────
      ...
      for (const sr of responses) {
        ...
        for (const ans of sr.answers) {
          ...
          if (ans.question.type === QuestionType.RATING) {
            ...
            val = ans.selectedOptions
              .map((o: any) => o.option.text)
              .join("; ");
          }
    ...
    // writeBuffer() returns an ArrayBuffer; we wrap it in a Node Buffer
    // and set Content-Length to prevent chunked-transfer corruption.
